<template>
    <div class="container mx-auto bg-gray-100 p-6 h-screen mb-20">
        <div class="text-4xl font-bold mb-6">Modification d'un produit</div>
        <form class="container mx-auto mb-12 lg:w-4/5 lg:mx-auto">
            ...
            <div class="md:flex md:items-center md:p-6">
                
                    <button 
                        @click="updateArticle"
                        id="button"
                        type="submit"
                        class="w-full mr-4 shadow bg-gray-900 hover:bg-gray-700 focus:shadow-outline focus:outline-none text-white font-bold mt-6 py-2 px-4 rounded">
                        Valider
                    </button>
                    <router-link to="/administration">
                        <button class="w-full shadow bg-gray-900 hover:bg-gray-700 focus:shadow-outline focus:outline-none text-white font-bold mt-6 py-2 px-4 rounded">
                            Retour
                        </button>
                    </router-link>
            </div>
...
export default {
    ...
    created() {
     ...
     methods: {
         getArticle(){
            axios.get('http://'+process.env.VUE_APP_SERVER_IP+":"+process.env.VUE_APP_PRODUCT_PORT+'/api/product/' + this.id)
            .then(response => {
         ...
         updateArticle(){
            ...
            axios.put('http://'+process.env.VUE_APP_SERVER_IP+":"+process.env.VUE_APP_PRODUCT_PORT+'/api/product/' + this.id, null, { params : paramsToUse})
            .then(response => {
